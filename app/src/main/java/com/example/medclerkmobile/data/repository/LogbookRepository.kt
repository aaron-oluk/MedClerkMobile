...
class LogbookRepository(private val api: ApiService) {
    ...
    suspend fun pendingReview(): Result<List<LogbookEntry>> = safeCall { api.pendingReview().data }
    ...
    suspend fun signOffEntry(id: Int): Result<LogbookEntry> = safeCall { api.signOffLogbookEntry(id) }
